...
const navBarSlice = createSlice({
  ...
  reducers: {
    ...
    selectSubItem: (state, action) => {
      ...
      if (state.subMenu[action.payload].isActive)
        state.subMenu[action.payload].isActive = false;
      else state.subMenu[action.payload].isActive = true;
    },
...
export const { selectSubItem, setActive, selectModalItem,goToSubMenu,goToMainMenu } =
  navBarSlice.actions;
