...
export const updateProductInfo = async(params:IUpdateProductInfo):Promise<IProduct[]>=>{
  try {
...
export const removeProductInfo = async(params:IRemoveProductInfo):Promise<IProduct[]>=>{
  try {
    const {data} = await $host.delete('api/products/removeInfo',{data:params});
    return data;
  } catch (error) {
    console.error(error);
    return error;
